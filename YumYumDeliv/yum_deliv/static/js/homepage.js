$(document).ready(function () {
    ...
    $('#add-address').click(function () {
        ...
        if (!$("#address-dialog").dialog('instance')) {
...
document.addEventListener('DOMContentLoaded', function () {
        ...
        link.addEventListener('click', function (event) {
            ...
            restaurantItems.forEach(item => {
                ...
                if (selectedType === '0' || itemType === selectedType) {
                    // Check search query and display accordingly
                    if (restaurantName.includes(searchQuery) || itemType === searchQuery) {
    ...
    document.getElementById('all_rest').addEventListener('click', function (event) {
        event.preventDefault();
        ...
        // Clear selected type and update labels
        typeLinks.forEach(typeLink => {
            typeLink.classList.remove('selected-type');
        });
        updateNoResultsMessage();
        updateH1Label('0'); // '0' represents "Все"
    });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function selectAddress(address, entrance, flat, floor, intercom) {
        // Serialize details into a JSON string
        var detailsObject = {
            entrance: entrance,
            flat: flat,
            floor: floor,
        ...
        var detailsString = JSON.stringify(detailsObject);

        // Save selected address and details in cookies
        setCookie("selectedAddress", address, 30);
        setCookie("selectedAddress-details", detailsString, 30);
    ...
    document.addEventListener('click', function (event) {
        ...
        if (target.getAttribute('data-type') === 'user-address') {
            ...
            var entrance = target.getAttribute('data-entrance');
            var flat = target.getAttribute('data-flat');
            var floor = target.getAttribute('data-floor');
            var intercom = target.getAttribute('data-intercom');
            selectAddress(address, entrance, flat, floor, intercom);
        }
    });
...
function calculateTotalCost() {
    ...
    var totalCost = 0;
    for (var i = 0; i < cart.length; i++) {
        totalCost += cart[i][0].quantity * parseInt(cart[i][0].cost);
    }
    return totalCost;
}